...
| **step 2** | the sort of the husky codes, the step RHSort replaces. This is its name: 20 uses and no definition needed | "the first pass" or "the first sorting pass", which three sites used until 2026-09-11 --- and which is doubly confusing, step 2 being the first *sorting* pass but the second step |
| **husky-sorting** | the whole algorithm, all three steps. The array-access total $A$ is quoted "for husky-sorting an array", and that formula carries the encoding and cleanup terms as well as the sort | a name for step 2 alone. Considered on 2026-09-11 and rejected: it would contradict both existing uses, and \textit{husky-sorting} sits one inflection from \textit{HuskySort}, so the two cannot carry opposite scopes without being conflated |

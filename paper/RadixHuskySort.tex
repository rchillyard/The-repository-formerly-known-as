...
These findings are not an artifact of a single machine.
The same qualitative pattern held on three environments spanning two CPU vendors and three
architectures (\S~\ref{sec:environments}), the last a 16-core ARM instance with no
performance/efficiency split to confound the result.
That run also settled a question the parallel variant's own results had left open
(\S~\ref{sec:parallel-radix}): its scaling ceiling past a handful of threads is a property of the
design --- the sequential histogram-combine once per pass --- not of any machine's mix of core types.
...
A fourth condition is about the machine rather than the workload: where cores are free,
\mbox{\textit{Arrays.parallelSort}} beats serial RadixHuskySort outright (\S~\ref{sec:parallel-baseline}).
But that moves algorithm and parallelism together, and the two separate: our own variant leads it by
1.19x at ten million \textit{Long}s, and where it trails, the causes given there are the
implementation's, not the approach's.
A parallel proxy-key sort should win wherever the serial one does; that ours does not yet is the
clearest future work here.

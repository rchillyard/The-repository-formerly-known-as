...
the pair.
If $x > y$ but $x.huskyCode() = y.huskyCode()$, the codes say nothing about the pair at all, so step 2
leaves it in whatever order it arrived in --- input order, RHSort's counting-sort passes being stable,
and an arbitrary order under QuickHuskySort's quicksort --- and it may be left inverted as a result.
...
This is the \textit{perfect} encoding of \S~\ref{sec:overview}, stated precisely: $p = 0$.
In such a case, we can skip step 3 of the algorithm because, when $p = 0$, there will be no inversions remaining after quick-sorting according to the husky codes.

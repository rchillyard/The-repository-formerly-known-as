...
Radix sort's independent per-digit counting-sort passes are well suited to parallelization across CPU threads,
...
same codes, and we add a parallel variant of it (\S~\ref{sec:parallel-radix});
...
\S~\ref{sec:parallel-radix} takes up parallelism on its own terms.
...
1.19x at ten million \textit{Long}s, trails it at two million, and on the permits trails its own serial
form (\S~\ref{sec:parallel-baseline}).
The causes named there are our implementation's, not the approach's, so a parallel proxy-key sort should
still win wherever the serial one does; that ours does not yet is the clearest future work here.

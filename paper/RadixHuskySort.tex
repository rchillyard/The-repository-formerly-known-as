...
many there were.
And the cost of the cleanup pass, though analysed in terms of the proportion of inversions the
encoding leaves behind, was never measured: it reports no calculated value for $p_{crit}$.
...
A \textit{perfect} encoding introduces no inversions at all, so that sorting the codes has already
...
This confines all object-reference movement to one pass in total, rather than one pass per digit.

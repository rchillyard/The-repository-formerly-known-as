...
thread counts, not against \mbox{\textit{Arrays.parallelSort}}, which we have not measured on the machine of
record.
...
\begin{table*}
  \caption{Which sort to reach for. The situations are ordered by how much they decide, and
    ...
    cores to spare, compare like with like, since \mbox{\textit{Arrays.parallelSort}} costs less to adopt than
    ParallelRadixHuskySort (\S~\ref{sec:parallel-radix}).}
  \label{tab:Guidance}
  \centering
...
And spare cores cut both ways.
\textit{ParallelRadixHuskySort} widens RadixHuskySort's advantage further still (\S~\ref{sec:parallel-radix}),
given enough work to amortize its thread-coordination cost —
but the baseline parallelizes too, and more cheaply:
\mbox{\textit{Arrays.parallelSort}} is one method call and no new code,
and with cores to spare it can overtake serial RadixHuskySort outright.
The comparison that settles the multicore case is parallel against parallel,
which we have not made on the machine of record.

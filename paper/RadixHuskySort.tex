...
(\S~\ref{sec:radix}).
Those passes, however, are not where a proxy-key sort spends its time.
The encoding and the cleanup are both sequential, and on the corpora we measure they dominate; so the
advantage this paper reports is a serial one, and the parallel variant is offered as a measured
negative result rather than as a feature.
\S~\ref{sec:conclusion} says why that is a property of the mechanism and not of our implementation
of it.
...
\textbf{The advantage this paper reports is a serial one, and that is a property of the mechanism
rather than a shortcoming of our implementation of it.}
...
It is tempting to read that as an implementation defect, and we did at first.
It is not.
Husky coding earns its advantage by moving work out of the linearithmic phase and into two linear ones,
the encoding and the cleanup, and both of those are sequential.
A sort that wins by performing less total work has, by construction, less work left to distribute
across cores.
...
names; and the encoding and the cleanup together --- neither of which any thread count touches --- already
cost more than \mbox{\textit{Arrays.parallelSort}} takes to do the entire job on both of those corpora.
Making the digit passes free and perfectly parallel would not close that gap.
Where parallelism is what matters and cores are plentiful, a parallel comparison sort is the right
tool, and we say so.
The sequential histogram-combine of \S~\ref{sec:parallel-radix} is a real defect and worth repairing,
but repairing it would not change this conclusion.

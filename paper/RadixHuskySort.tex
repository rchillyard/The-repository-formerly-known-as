...
%%
%%   pdflatex RadixHuskySort                                 -> identified, draft stamp on
%%   pdflatex -jobname=RadixHuskySort-anon "\def\ANONYMOUS{}\input{RadixHuskySort}" -> blind
%%
...
%% [listings removed 2026-09-12 with the last lstlisting]
\usepackage{graphicx}
\usepackage{latexsym}
\usepackage{etoolbox}
...
%% [mystyle listing style removed with the listings package]
...
\ifanonymous
  %% Not \author{}: an empty one still gets \@maketitle's skips on both sides of an empty
  %% tabular, which left 66pt of nothing between the title and the abstract -- a hole in the
  %% one copy reviewers actually read. "Anonymous Author(s)" is the conventional
  %% double-blind placeholder and occupies the space the names will occupy at camera-ready,
  %% so the blind and identified builds have the same shape.
...
\begin{abstract}
    ...
    Measured against Java's system sort for objects (\textit{Arrays.sort}) --- integers,
    ...
    Two things overtake us: on English words a tuned MSD radix sort, and, where cores are free,
    the JDK's own parallel sort.
\end{abstract}
...
Counting memory operations rather than only comparisons is an established idea, and not one we claim
as new.
Three lines of work bound ours: the external-memory (I/O) model of Aggarwal and
Vitter~\cite{10.1145/48529.48535}, which counts block transfers between disk and memory;
the cache-oblivious algorithms of Frigo et al.~\cite{814600}, which reach near-optimal
memory-hierarchy behaviour without advance knowledge of the cache parameters;
and, closest to our own concern, LaMarca and Ladner's study of how caches shape the relative
performance of sorting algorithms~\cite{LAMARCA199966}.
All three are chiefly concerned with data too large to fit in memory, or in cache, at all.
Our array-access count is a simpler, single-level analogue: we count raw array reads and writes as a
proxy for real cost, motivated by the observation that comparing two heavyweight objects --- two
\textit{String}s, say --- touches more memory, and more likely uncached memory, than comparing two
64-bit primitives.

Radix sort's independent per-digit counting-sort passes are, taken alone, well suited to
parallelization across CPU threads, a property already established for radix sort generally, and
\S~\ref{sec:parallel-radix} describes and benchmarks a parallel variant of RadixHuskySort
(\S~\ref{sec:radix}).
Those passes, however, are not where a proxy-key sort spends its time: the encoding and the cleanup
dominate on the corpora we measure, and we did not parallelize either.
So the advantage demonstrated here is a serial one, and the parallel variant is reported as a measured
negative result rather than offered as a feature.
\S~\ref{sec:conclusion} gives the figures, and is careful to distinguish that result --- which bounds
our implementation --- from any claim about the mechanism, which we do not make.
We do not make even the narrow claim for QuickHuskySort's own comparison-based steps:
...
While sorting appears to be a settled topic, new improvements are occasionally made:
\cite{Dual-pivot,Timsort,doi:10.1137/1.9781611975994.101,10.5555/313559.313859,10.1002/spe.4380231105}.
Indeed, there is an abundance of sorting techniques suitable for a myriad of different use cases.
Nevertheless, there is still room for improved versions in regard to computation, memory, time, and space complexity.
In a recent paper~\cite{sort}, the authors conclude that quicksort is the fastest general-purpose sorting algorithm,
...
Timsort, a hybrid stable sorting algorithm derived from merge sort, is the default object-sorting utility in the Python and Java libraries.

The husky code was introduced by Hillyard et al.~\cite{huskysort2020}, together with QuickHuskySort,
which sorts an array of objects by sorting an array of their codes and then repairs any remaining inversions among the objects.
...
Its step 2 was a comparison sort, so the encoding made each comparison cheap without reducing how
many there were.
And the cost of the cleanup pass, though analysed in terms of the proportion of inversions that remain after step 2,
was never measured: it reports no calculated value for $p_{crit}$.
The contributions here answer both.
RadixHuskySort, or RHSort (\S~\ref{sec:radix}), replaces step 2 with a linear-time radix sort on the
same codes; we also parallelize its digit passes, which turns out not to pay and is reported as
such (\S~\ref{sec:parallel-radix});
\S~\ref{sec:pcrit} measures what the cleanup pass costs.
Alongside those, an empirical comparison against the classic string sorts rather than a theoretical
one (\S~\ref{sec:radix}), and a case study on real rather than generated data
...
space and of being no faster than $\textbf{O}(N \log N)$ on unordered input, but adaptive, such that where the
...
\renewcommand{\labelenumi}{\alph{enumi}}
    ...
    \end{enumerate}
\item {}Sort the \emph{longs} array --- with dual-pivot quicksort in the original algorithm, with an
insertion-sort cutoff and the heapsort fallback of \S~\ref{sec:background} ---
in such a way that when elements $i, j$ of \emph{longs} are exchanged, then elements $i, j$ of $xs$ are also exchanged;
\item {}Finally, when step 2 is finished and, \emph{if necessary},
we run Timsort on the array $xs$ to ensure that its elements are truly in order.
\end{steps}

Step 3 is needed only where the encoding is imperfect.
A \textit{perfect} encoding leaves no inversions at all, so that sorting the codes has already
sorted the elements and step 3 can be skipped;
\S~\ref{sec:coding} states the condition precisely and \S~\ref{sec:pcrit} measures what an
imperfect one costs.
...
Step 2's quicksort is $\textbf{O}(N \log N)$, but the codes it sorts are fixed-width 64-bit values,
...
So step 2 can be linear too.
This section describes RadixHuskySort, which makes it so (Algorithm ~\ref{alg:radixhuskysort}).
...
Instead, RadixHuskySort's step 2 carries only an $\textit{int[]}$ index array through the LSD digit
passes (Algorithm ~\ref{alg:radixhuskysort}), alongside the (unmodified) long array;
each pass is a textbook stable counting sort~\cite{CLRS} keyed on one $b$-bit digit of the
(already-computed) husky code:
a histogram of how many indices carry each of the $2^b$ digit values, a prefix sum turning that into a
starting offset per value, then a scatter of the indices into their new order ---
...
This confines all object-reference movements to one pass in total, rather than one pass per digit.
...
the same radix sort would apply unchanged, with twice as many passes.

Width alone does not buy perfection, though:
only a bounded domain admits a perfect fixed-width code at all,
and a coder limited by its own information rather than by its width --- pinyin, whose few hundred
syllables leave true homonyms at any width --- gains nothing from extra bits.
Nor are those passes free, and what they buy is all-or-nothing, since step 3 is skipped only for a
perfect encoding: the measured shares put a single pass at roughly a seventh of RHSort's running time
against a cleanup pass of at most a sixth (\S~\ref{sec:pcrit}), so the trade pays only where one or
two further digits would reach perfection and not at all where they would merely approach it.
...
and no specialised competitor --- and yield the largest margin in the table at 5.5x.
...
MSD earns it on a corpus that suits it, and the limits that keep it off the others are ours rather
than the algorithm's: our alphabet reserves 256 positions for characters beyond ASCII, which English
text fits and neither Chinese corpus does, and the collation we gave it is code-point order rather
than pinyin.
Neither ceiling is inherent --- a wider alphabet and a pinyin-aware MSD are both implementable, and we
did exactly that for the multikey baseline below --- so the absent Chinese rows record work we did not
do, not a limitation of MSD radix sort.
...
RHSort is faster than the repaired multikey quicksort by 1.54x, 1.80x and 2.08x at $N = 32{,}000$,
$200{,}000$ and $1{,}000{,}000$, and QuickHuskySort by 1.33x, 1.49x and 1.27x.
Two independent runs on the machine of record agree to within seven hundredths of those multipliers.
...
\KwOut{\it{xs}, sorted}
 coding = huskyCoder.huskyEncode(xs) \tcp*{Algorithm ~\ref{alg:huskyencode}}
 \tcp{flip the sign bit, so unsigned digit order matches signed order}
 biased[i] = coding.longs[i] {\bf XOR} Long.MIN\_VALUE, for each i\;
 index = [0, 1, \ldots, xs.length - 1]\;
 \For{digit = 0 \KwTo (64 / b) - 1}{
   index = stableCountingSortPass(biased, index, digit, b)\;
 }
 \tcp{the only pass that moves object references}
 source = copy of xs\;
 xs[i] = source[index[i]], for each i\;
 \lIf{{\bf not} coding.perfect}{Arrays.sort(xs)}
 \Return xs\;
 \caption{RadixHuskySort.sort. The digit passes reorder only \textit{index}, an \textit{int[]}, so
   the payload is permuted once at the end rather than once per digit.}
 \label{alg:radixhuskysort}
\end{algorithm2e}
...
Step 1b requires the definition of a 64-bit husky code (refer to Algorithm ~\ref{alg:huskyencode}).
Its defining property is monotonicity: if $x$ and $y$ compare equal then
$x.huskyCode() = y.huskyCode()$, and if $x > y$ then $x.huskyCode() \geq y.huskyCode()$ --- the
latter holding with probability $1 - p$ and failing with probability $p$.
Note that the relation that matters is \textit{compareTo}, not \textit{equals}: the two differ for
\textit{BigDecimal}, whose coder is \textit{longValue}, and it is the ordering the sort must respect.

The two ways the second condition can fail differ.
If $x > y$ but $x.huskyCode() < y.huskyCode()$, the codes are positively wrong and step 2 will invert
the pair.
If $x > y$ but $x.huskyCode() = y.huskyCode()$, the codes say nothing about the pair at all, so
whatever order step 2 leaves it in is arbitrary with respect to the true one, and it may be inverted.
Only step 3 can settle either case.
...
Leaving a more detailed discussion of $p_{crit}$ until later, we note that, for some domains of $X$, we
can definitely assert that $p = 0$ --- which is the \textit{perfect} encoding of \S~\ref{sec:overview},
stated precisely.
In such a case we can skip step 3, because with $p = 0$ no inversions remain after sorting the husky
codes.
...
In any event, assuming that the huskyCode is as accurate as required, then there will be very few inversions (elements out of place) remaining after step 2.
...
\label{sec:pcrit}
When $p > 0$, there is a finite probability of any pair of elements remaining inverted after step 2.
For an array of N randomly ordered elements, the average number of inversions before sorting is $X = N (N -1 ) / 4$.
The number of inversions remaining after step 2 is simply $p X$.
...
\[T_1 = k_1 N\]
\item{}  $T_2$ is the (average) time for step 2, sorting the husky code array while permuting the
element array alongside it, with a comparison sort:
\[T_2 = 2 k_2 N \ln N\]
\item{}  $T_3$  is the time to Timsort the element array (only required if $p>0$):
 \[T_3 = k_3 (N + p X)\]
\end{steps}
Provided that the total time $T = T_1 + T_2 + T_3$ is less than the time required to use Timsort on the original element array,
the method wins.
...
the system sort compares these same records 3.25 million times in 151\,ms, or 46\,ns each,
so the cleanup pass at 74\,ns costs about 1.6 times as much per comparison using the same comparator.
Step 2 permutes references without moving the records, so neither sort walks the heap in order ---
but the system sort touches each record some thirty times as a comparison operand, where the cleanup
pass touches it twice.
A cache miss is amortised over the first and paid in full by the second.
...
Second, that cost is fixed rather than proportional to the work it does.
An encoding that can be declared perfect skips the pass entirely; one that cannot pays $N-1$ full
composite comparisons --- up to a quarter of the running time --- before correcting a single genuine
inversion.
...
first character is \texttt{0x8000} or above --- common among Chinese characters --- would otherwise set the
...
\KwOut{a \it{Coding}: the codes, and whether they are perfect}
 result = new long[xs.length]\;
 perfect = true\;
 \For{i = 0 \KwTo xs.length - 1}{
   result[i] = huskyEncode(xs[i])\;
   \lIf{perfect}{perfect = perfectFor(xs[i])}
 }
 \Return new Coding(result, perfect)\;
 \caption{huskyEncode. Perfection depends on the data, not only on the coder, so it is settled as the
   codes are computed. \textit{perfectFor} tests one element in constant time: a length comparison for
   a string, the coder's own constant where perfection is data-independent.}
 \label{alg:huskyencode}
\end{algorithm2e}
...
%% ANONYMISATION: the real URL names its owner, so it is guarded with the author block. The blind
%% branch cites the Anonymous GitHub mirror of a pruned snapshot (source, tests and the JMH harness
%% only -- no doc/, no paper/, no history); the identified branch keeps the real repository, which
%% is what the camera-ready wants. See "0i. The repository link" in Paper edits pending.md.
\ifanonymous
  The implementation, its tests and the benchmark harness are available at
  \url{https://anonymous.4open.science/r/radix-proxy-key-sort-artifact-D8B1/}.
\else
...
The English and Chinese strings come from the Leipzig Corpora Collection~\cite{GOLDHAHN12.327}, taking the appropriate
\emph{sentences.txt} file and splitting each line on whitespace and punctuation.
Chinese personal names, used for the pinyin-order comparisons of \S~\ref{sec:radix-results}, are a
separate corpus~\cite{wainshine2020chinesenames}.
The building-permit records of \S~\ref{sec:radix-results} are 198,900 permits filed between January
2013 and February 2018, taken from San Francisco's published record~\cite{sf-building-permits} and
ordered by a composite key of assessor's block, lot and filing date; only those three columns are
retained, and the data is used under the Open Data Commons Database Contents License.
They are included because their distribution is the city's rather than ours.
...
At $N = 1{,}000{,}000$, $A_r \approx 27{,}000{,}000$ against $A_h \approx 97{,}000{,}000$ and $A_m \approx 161{,}000{,}000$ —
a predicted advantage of roughly 3.6x over QuickHuskySort,
...
but it is worth stating rather than leaving the model looking like radix wins unconditionally at every $N$.
 \begin{table*}
  ...
  \begin{tabular}{rrrrr}
    ...
    \midrule
     4 & 6 & 72 & 71 & 108 \\
     1,000 & 6,908 & 81,439 & 53,210 & 27,000 \\
     1,000,000 & 13,815,511 & 160,878,371 & 97,419,268 & 27,000,000 \\
     1,000,000,000 & 20,723,265,837 & 240,317,557,125 & 141,628,901,356 & 27,000,000,000 \\
...
Throughout, \emph{system sort} means \textit{Arrays.sort}, the JVM's default for objects, and every
sorter in the table is single-threaded.
At the larger sizes a practitioner who cared about speed would reach instead for
\mbox{\textit{Arrays.parallelSort}}, so the serial comparison is the conservative one to draw against
RadixHuskySort and the generous one to draw against a parallel sorter;
\S~\ref{sec:parallel-baseline} draws the parallel-against-parallel comparison, and
\S~\ref{sec:parallel-radix} reports what parallelizing our own digit passes does and does not buy.
...
\begin{table*}
  ...
  \begin{tabular}{llllll}
    ...
    Dates & 20,000 & 3.793$\pm$0.007 & --- & 0.836$\pm$0.002 & 0.789$\pm$0.002 \\
...
\begin{table*}
  \caption{RHSort's advantage over the comparison-based husky sort it replaces, as a direct
    multiplier. Every row uses RHSort/16, so those that appear in Table ~\ref{tab:HS_BM} can be
    checked by dividing its two husky columns. Eleven-bit digits are a few per cent faster for
    \textit{Long} and for dates, within the plateau of \S~\ref{sec:radix-results}, but a table
    mixing widths would match nothing printed. The dates baseline partitions three ways rather than
    on two pivots.}
...
RHSort's advantage over QuickHuskySort likewise does not shrink with $N$:
...
Dates show the largest margin (5.5x) because that coding is provably perfect and never needs a cleanup pass at all,
...
\begin{table*}
  \caption{Which sort to reach for. The situations are ordered by how much they decide, and
    array size settles nothing until all of them are. Every sorter named here is single-threaded; with
    cores to spare, \mbox{\textit{Arrays.parallelSort}} wins outright on strings and on the permits
    (\S~\ref{sec:parallel-baseline}).}
  \label{tab:Guidance}
  \centering
...
Your situation & Reach for & Evidence \\
    ...
    The ordering is composite or expensive, \emph{and} the key packs exactly into 64 bits
      ...
      & the largest margins we measure: dates 5.5x, building permits 2.1x over QuickHuskySort \\
    ...
    the coder can see
      & System sort --- \mbox{\textit{Arrays.parallelSort}} if cores are free
      & every husky variant loses its advantage (\S~\ref{sec:adversarial}) \\
...
Every one of these crossovers shares the same shape: each successive algorithm carries a larger fixed
setup cost that pays for itself only once there is work enough to amortize it —
the story \S~\ref{sec:parallel-radix} finds one level down, in ParallelRHSort's thread and barrier setup.
They were measured for String keys only.
The costs involved are architectural rather than type-specific, so other key types plausibly show
similar curves at similar sizes, though we have not confirmed it.
...
And spare cores change the answer, which we now measure rather than concede
(\S~\ref{sec:parallel-baseline}).
\mbox{\textit{Arrays.parallelSort}} is one method call and no code of ours, and on sixteen cores it beats
serial RadixHuskySort by 2.98x on a million English strings and 2.35x on the permits.
Parallel against parallel, \textit{ParallelRadixHuskySort} leads by 1.19x at ten million \textit{Long}s,
trails by 1.25x at two million, and on the permits never beats its own serial form.
Where cores are free and the JDK can compare the key, reach for \mbox{\textit{Arrays.parallelSort}};
our variant earns its place on large arrays whose ordering the encoding replaces.

\section{Conclusion.}
\label{sec:conclusion}

The original idea behind HuskySort — QuickHuskySort — was to shift work from the linearithmic phase of
sorting into the linear ones, prelude and postlude, reducing array accesses and so processing time.
...
Strings are not reliably that case: the one domain with a mature specialised literature of its own
(\S~\ref{sec:radix}), and the most variable, spanning nearly the whole range of our results ---
including our narrowest margin, Chinese personal names in pinyin order.

For RadixHuskySort the improvement is larger still above a few thousand elements, and since that
variant has no linearithmic phase at all, the saving comes from bit manipulation replacing calls to
\textit{equals}.

These findings are not an artifact of a single machine: the same qualitative pattern held on three
environments spanning two CPU vendors and three architectures (\S~\ref{sec:environments}), the last a
16-core ARM instance with no performance/efficiency split to confound it.
That run also settled a question the parallel variant had left open (\S~\ref{sec:parallel-radix}): its
scaling ceiling past a handful of threads is a property of the design --- the sequential
histogram-combine once per pass --- not of any machine's mix of core types.

HuskySort's advantage is specific to a kind of workload rather than universal, and needs all three of
the following: an array of at least a few hundred elements, growing to a few thousand before
RadixHuskySort's per-pass setup cost is repaid (\S~\ref{sec:usecase});
no expectation of pre-existing order, which favours Timsort's adaptivity instead (\S~\ref{sec:summary});
and a comparison costing more than a machine-word check --- strings, arbitrary-precision numbers
and multi-field records, as opposed to \textit{int}, \textit{long} or \textit{double},
where dual-pivot quicksort wins outright.

\textbf{The advantage demonstrated here is a serial one.}
Parallel against parallel, \textit{ParallelRadixHuskySort} leads \mbox{\textit{Arrays.parallelSort}} by
1.19x at ten million \textit{Long}s --- the cheapest comparison we measure, and so the case where
husky coding has least to offer --- trails it at two million, and on the permits trails its own serial
form (\S~\ref{sec:parallel-baseline}).
The reason is specific, and it is worth stating exactly, because it is easy to state too strongly in
either direction.
A proxy-key sort has four phases: the encoding, the digit passes, the permutation, and the cleanup.
We parallelized two of them, the encoding and the digit passes, and the two that remain are between
them the larger share of the time.
Measured, the parallelizable fraction of the whole sort is 0.06 on English words and 0.07 on Chinese
names, so no thread count reaches past roughly a 7\% improvement; and the encoding and the cleanup
together already cost more than \mbox{\textit{Arrays.parallelSort}} takes to do the entire job on both
of those corpora.
Making the digit passes free and perfectly parallel would therefore not close that gap.

What that does \emph{not} establish is a limit on the approach.
The other three phases are parallelizable too: the encoding is a pure function of each element, the
permutation is an independent scatter under a bijection, and the cleanup --- contrary to our own
expectation --- parallelizes well, \mbox{\textit{Arrays.parallelSort}} beating a serial Timsort by 1.75x
even on an array already in order, since it performs about four times the work across seven workers.
So the ceiling reported above bounds this implementation rather than the mechanism.
Whether a fully parallel proxy-key sort beats a parallel comparison sort is an open question with a
calculable budget: pre-ordering saves the baseline about 69\,ms at a million English strings, and a
sequential encoding of those strings costs 73\,ms on its own --- so it does not fit until it is
divided, which is why we divide it.
Across eight cores the encoding falls to 21\,ms, leaving some 48\,ms for the digit passes and the
permutation.
We have not measured whether those fit, and so claim neither outcome.
Parallelizing the permutation and the cleanup, and repairing the sequential histogram-combine of
\S~\ref{sec:parallel-radix}, is the clearest future work here.
Where parallelism is what matters and cores are plentiful, a parallel comparison sort is today the
right tool, and we say so.
...
Wild and Nebel's is the right analysis to apply here: step 2's partitioning is itself dual-pivot
(\S~\ref{sec:overview}).
...
\subsection{\texorpdfstring{\textit{Arrays.parallelSort}}{Arrays.parallelSort} as a parallel baseline.}
\label{sec:parallel-baseline}

Every sorter in Table ~\ref{tab:HS_BM} is single-threaded, and at the sizes reported there a
practitioner who cared about speed would not call \textit{Arrays.sort} at all.
They would call \mbox{\textit{Arrays.parallelSort}}, which is one method call away and needs no code of
ours.
This section reports that comparison on the machine of record.
It is the comparison a practitioner faces; it is not a ceiling test of our design, because
\mbox{\textit{Arrays.parallelSort}} draws on the common \textit{ForkJoinPool} --- fifteen workers plus
the calling thread on this machine's sixteen cores --- while \textit{ParallelRadixHuskySort} was
measured at four and at eight fixed threads, and no fifteen- or sixteen-thread configuration of it was
run.
These runs used three forks rather than the five of every other measurement here, so the intervals are
wider; the host also carried unrelated load, and the one row visibly affected is called out below.

On strings the baseline wins outright, and by a wide margin:
at $N=200{,}000$ it takes 9.94$\pm$0.50\,ms against serial RHSort/16's 51.42$\pm$3.06, and at
$N=1{,}000{,}000$ 92.83$\pm$2.53 against 276.89$\pm$7.23 --- faster by 5.17x and 2.98x.
Its own advantage over \textit{Arrays.sort} at $N=200{,}000$ is about fourteenfold, a little under one
factor per thread, which is what a well-parallelized merge sort on fifteen workers should give.

\begin{table*}
  \caption{\textit{ParallelRadixHuskySort} against \mbox{\textit{Arrays.parallelSort}} on
    \textit{Long[]} (ms, mean $\pm$ 99.9\% CI, 11-bit digits). All columns are from one run, so the
    husky figures differ slightly from Table ~\ref{tab:ParallelRadix}, which reports a cleaner
    five-fork run without this baseline. The $p1$ figure at ten million is the row affected by host
    load and should be read with its interval.}
  \label{tab:ParallelBaseline}
  \centering
  \begin{tabular}{ccccccc}
    \toprule
    N & Serial & 1 thread & 2 threads & 4 threads & 8 threads & \textit{Arrays.parallelSort} \\
    \midrule
    2,000,000 & 172.8$\pm$7.7 & 168.7$\pm$3.3 & 140.7$\pm$1.6 & 118.9$\pm$2.2 & 112.8$\pm$2.5 & 90.1$\pm$5.2 \\
    10,000,000 & 932.4$\pm$35.7 & 990.9$\pm$111.7 & 751.0$\pm$54.9 & 670.6$\pm$30.4 & 644.4$\pm$32.9 & 765.1$\pm$11.3 \\
    \bottomrule
  \end{tabular}
\end{table*}

On \textit{Long[]} the answer depends on the size, and the crossover falls between the two we measured.
At $N=2{,}000{,}000$ the baseline is ahead: 90.1$\pm$5.2 against our best row's 112.8$\pm$2.5, so eight
threads is 1.25x \emph{slower}, with disjoint intervals.
At $N=10{,}000{,}000$ the order reverses and eight threads leads by 1.19x, 644.4$\pm$32.9 against
765.1$\pm$11.3, again disjoint.
The whole of step 2 was run a second time on the same jar: both verdicts reproduced, 1.27x slower at two
million and 1.16x faster at ten, each with disjoint intervals.
Only the four-thread margin at ten million failed to reproduce, falling from 1.14x to 1.06x with
intervals that overlap, so that one is not established.

\begin{table*}
  \caption{The permits under both parallel sorts (ms, mean $\pm$ 99.9\% CI). RHSort/16 is serial.}
  \label{tab:ParallelPermits}
  \centering
  \begin{tabular}{cccccc}
    \toprule
    N & System sort & \textit{Arrays.parallelSort} & RHSort/16 & 4 threads & 8 threads \\
    \midrule
    32,000 & 13.64$\pm$0.06 & 4.62$\pm$0.03 & 2.72$\pm$0.09 & 3.34$\pm$0.07 & 3.42$\pm$0.07 \\
    100,000 & 62.05$\pm$1.17 & 6.13$\pm$0.08 & 14.82$\pm$0.92 & 16.00$\pm$1.09 & 16.57$\pm$0.49 \\
    198,900 & 152.06$\pm$2.76 & 13.13$\pm$0.15 & 30.89$\pm$1.21 & 32.65$\pm$2.37 & 36.38$\pm$0.54 \\
    \bottomrule
  \end{tabular}
\end{table*}

The permits are the least flattering case and the most instructive, because the mechanism is legible.
Our own serial RHSort/16 leads the baseline at $N=32{,}000$ by 1.69x, but by the full corpus the
baseline is 2.35x faster than it, 2.49x faster than four threads and 2.77x faster than eight, every
interval disjoint --- so the crossover sits between 32,000 and 100,000.
Worse for the parallel variant: \emph{at no size does it beat its own serial form}.
Three properties of the implementation account for it, and all three are design choices rather than
properties of the data.
Passes are split into \mbox{$\max(1, \min(\textit{parallelism}, N / 2^{14}))$} chunks, so at
$N=32{,}000$ both configurations run a single chunk --- which is why their two rows agree --- and eight
threads are reached only at the full corpus.
The thread pool is constructed inside the timed region, so its start-up is charged to every
measurement.
And at 16-bit digits each chunk builds a 65,536-entry histogram per pass, which at these sizes is more
buckets than elements; bookkeeping that scales with chunks times buckets rather than with $N$ would fit
losing here while gaining on the two-to-ten-million \textit{Long[]} arrays, where each chunk holds
hundreds of thousands of elements and the digit is 11 bits wide.
We have not isolated the three contributions, so this is a reading of the design rather than a
measurement of it.
The serial result is unaffected throughout: the system sort remains 4.92x slower than serial RHSort/16
at the full corpus.

%% Moved out of the body (was \S 6.4 under Results and Observations) on 2026-09-17: the
%% parallel variant is a measured negative result rather than a feature, so it belongs beside
%% the parallel baseline it loses to, and the body has no page to spare. See TODO.md item 39.
...
The comparison here is against RadixHuskySort's own serial implementation and against itself at other
thread counts; \S~\ref{sec:parallel-baseline} sets it against \mbox{\textit{Arrays.parallelSort}} on the
same machine, which is the comparison a practitioner actually faces.
...
Unlike on machine B's eight cores, scaling does not stop at four threads:
on sixteen uniform cores, eight threads beats four at both sizes,
reaching 1.51x and 1.37x over a single thread.
It is resolved at $N=2{,}000{,}000$; at $N=10{,}000{,}000$ the intervals just touch,
so that size shows the direction rather than settling it.

That is still well short of linear, and the shortfall is the more interesting number:
an eightfold increase in threads buys between 1.37x and 1.51x,
on hardware with no performance/efficiency asymmetry to account for it.
Two effects fit, and they are not equally benign.
Each pass scatters across the whole array, so the sort may be limited by memory throughput, which extra
cores do not supply: a property of the machine.
The other belongs to the design --- the histogram-combine between a pass's two phases is sequential and
costs a step per bucket per chunk, so its work \emph{grows} with the thread count while the parallel
work shrinks.
That has a consequence the first lacks: the design should fail wherever buckets times threads approaches
$N$, and \S~\ref{sec:parallel-baseline} finds exactly that on the permits.
...
Against the system sort, RadixHuskySort is faster by 4.73x, 4.26x and 4.53x at $N = 32{,}000$,
$100{,}000$ and $198{,}900$; against QuickHuskySort, by 2.30x, 2.06x and 2.12x.
...
There is one comparison this case study loses, and it should be met here rather than left to
\S~\ref{sec:parallel-baseline} alone.
Given sixteen cores, \mbox{\textit{Arrays.parallelSort}} sorts the full corpus in 13.1\,ms against serial
RHSort/16's 30.9 --- 2.35x faster, and faster at that size than either parallel husky configuration.
Our own sort leads it at $N = 32{,}000$ by 1.69x, so the crossover falls between 32,000 and 100,000;
and since the corpus stops at 198,900 records, real data cannot say whether a second crossover lies
beyond it.
Two things follow, and they should not be run together.
The claim this case study makes is untouched: an encoding that replaces an expensive composite
comparison pays for itself, and in the very run that the baseline wins, RadixHuskySort is still 4.92x
faster than the system sort on equal terms.
What the result bounds is the recommendation.
Two hundred thousand records give a parallel merge sort ample room to divide, while our own chunking does
not reach eight threads until the whole corpus and builds a 65,536-entry histogram per chunk per pass at
this digit width.
So on a corpus of this size with cores to spare, \mbox{\textit{Arrays.parallelSort}} is the right call,
and closing that gap is a matter of our implementation rather than of the encoding.
...
RHSort's timings stay essentially flat across the entire sweep, exactly as the model of \S~\ref{sec:radix} predicts:
...
However, we have studied the efficacy of a modified merge sort (it is required to manage both the objects and the longs) as step 2.

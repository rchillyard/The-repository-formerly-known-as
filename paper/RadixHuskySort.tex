...
We parallelized one of them, the digit passes, and it is not the phase with the time in it.
...
calculable budget: pre-ordering saves the baseline about 69\,ms at a million English strings, of which
a measured encoding spends 24\,ms, leaving some 45\,ms for the passes and the permutation.
We have not measured whether they fit, and so claim neither outcome.
Parallelizing the remaining three phases, and the sequential histogram-combine of
\S~\ref{sec:parallel-radix} with them, is the clearest future work here.

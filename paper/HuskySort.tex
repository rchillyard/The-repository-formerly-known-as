...
\begin{abstract}
    Most sorting algorithms are judged by the comparisons or exchanges they perform.
    We argue the better yardstick is the total number of array accesses,
    which for a divide-and-conquer sort falls into a linear phase and a linearithmic one:
    moving work out of the second and into the first reduces the total,
    and where comparison is itself expensive it reduces running time as well.
    The mechanism is a 64-bit code, order-preserving as far as possible,
    standing proxy for each element; we call it a husky code.
    ...
    RadixHuskySort is faster in every case, by 3.6 to 6.8x at the largest size measured for each.
    The margin is widest where the ordering is expensive to evaluate and the encoding is exact;
    where the encoding is inexact we measure directly, for the first time,
    what the resulting cleanup pass costs.
...
They are 198,900 rows of San Francisco's published building-permit record, obtained from a public
redistribution of it\cite{sf-building-permits}, ordered by a composite key of assessor's block, lot and
filing date, and included because their distribution is the city's rather than ours.
...
\begin{table*}
  \caption{Sort times by data type (JMH, ms, mean $\pm$ 99.9\% CI). Numeric types at $N=500{,}000$.
    ...
    natural code-point order is a different and much cheaper task (\S~\ref{sec:radix-results}).}
  \label{tab:HS_BM}

...
Timsort for objects — for the types of objects that are expensive to compare:
...
One of the authors had originally posed this as a hypothetical question in a mid-term exam (Fall 2018):
encode a date-time object as a 64-bit hash code and sort the array of codes with quicksort.
It turned out to be a pretty good idea,
and we decided to test the efficacy of combining quicksort with Timsort for object types that are costly to compare.
...
Table ~\ref{tab:SysEnvAWS} lists a third,
independently run configuration used to confirm the radix-sort and parallel-scaling findings of \S~\ref{sec:radix-results} and \S~\ref{sec:parallel-radix} at larger scale:
a cloud server instance which, while also ARM-based like the machine of Table ~\ref{tab:SysEnvCurrent},
uses an entirely different core design, vendor, core count, and operating system, confirmed idle before the run.
...
A follow-up confirmation run rules out that explanation, however:
...
These crossover points were measured only for String keys, and only on the machine of Table ~\ref{tab:SysEnvCurrent}.
...
but neither that nor the exact thresholds on different hardware has been separately confirmed —
the AWS confirmation run of \S~\ref{sec:radix-results}--\S~\ref{sec:parallel-radix} only tested $N \ge 32{,}000$,
outside the range where these crossovers occur.
...
All three of the following need to hold:
\begin{itemize}
\item {}\textbf{A large array}: at least a few hundred elements for QuickHuskySort,
...
\item {}\textbf{No expectation of pre-existing order}: a partially-sorted array favors Timsort's adaptivity instead (\S~\ref{sec:summary}).
\item {}\textbf{A comparison or equality test that costs more than a single machine-word check}:

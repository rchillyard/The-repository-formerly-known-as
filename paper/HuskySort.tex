...
subject.
Two things left undone define the work here.
...
And while it measured the cleanup pass in one respect, choosing between Timsort and insertion sort
(\S~\ref{sec:cleanup-choice}), and analysed its cost in terms of the proportion of inversions the
encoding leaves behind, it never measured that proportion --- of the critical value $p_{crit}$ it says
plainly that no typical figure had been calculated (\S~\ref{sec:pcrit}).
...
\renewcommand{\labelenumi}{\alph{enumi}}
    ...
    \end{enumerate}
\item {}Sort array \emph{longs} using quicksort (in practice a dual-pivot quicksort, with an
insertion-sort cutoff below eighteen elements and Introsort's depth-limited heapsort
fallback~\cite{doi:10.1002})
...
Step 2 sorts the husky codes with that quicksort, an $\textbf{O}(N \log N)$ algorithm,

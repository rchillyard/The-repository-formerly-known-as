...
Chinese personal names are unfavourable on the second, and why is worth stating, since the two Chinese
rows otherwise look like two points on one axis.
The comparison for names is more expensive still, a table lookup per character; but the coders spend
their 64 bits very differently.
The Unicode coder gives each character sixteen bits drawn from thousands of code points, and a word of
one to four characters is often captured whole.
The pinyin coder gives each character nine bits for a syllable out of a few hundred, plus a tone out of
five --- and names are two or three characters whose surnames come from a smaller pool again.
Distinct codes are therefore few, ties common, and the cleanup pass does real work rather than none.
...
Leaving the word corpus in natural order is deliberate: a directory of people has one conventional
ordering and it is by pinyin, where a word list has several --- by radical, stroke count, pinyin or
frequency, according to purpose --- so code-point order there is one collation among many rather than
the wrong one, and it keeps that row comparable with the English one.
...
It does: HuskySort, and now RHSort, beat the system sort on all three,

...
for types where comparison itself is expensive, reduces processing time as well.
...
We present two algorithms based on this idea:
QuickHuskySort encodes each object into its husky code in a linear preamble to the sorting process,
then it sorts on the cheap encoded keys instead of the expensive originals,
and falls back to a cleanup pass only in those situations where the encoding is imperfect.
RadixHuskySort replaces that sort with a linear-time radix sort on the same encoded keys,
...
Timsort for objects — for the types of objects expensive to compare:
QuickHuskySort by 1.2--2.1x, RadixHuskySort by 2--6x at realistic array sizes,
a pattern confirmed independently across three CPU architectures.
\end{abstract}
...
stability, extra memory, partitioning, merging, exchanging (swapping), copying, and, of course, the comparison itself.
...
touches far more memory than a comparison between two 64-bit primitives.
...
a property that is well established for radix sort generally;
\S~\ref{sec:parallel-radix} describes and benchmarks a parallel variant of RadixHuskySort (\S~\ref{sec:radix}).
We do not make an equivalent claim for Huskysort's own comparison-based steps:
Introsort's partitioning and Timsort's adaptive run-detection are both substantially more data-dependent and sequential in character than radix sort's fixed per-digit passes,
so parallelizing them well is a separate, harder problem that this paper does not address.

Even though sorting appears to be a settled area of research, yet new improvements are still being made:
...
Before introducing Huskysort, it is worth laying out the four comparison-based algorithms it draws on side by side,
since each contributes a different property to the final design.
Table ~\ref{tab:PriorAlgorithms} summarizes their standard complexity and stability characteristics.
...
Quicksort is fastest in practice on unordered data (its actual average case),
because its in-place partitioning and cache-friendly access pattern outweigh merge sort's guaranteed $\textbf{O}(N \log N)$ bound;
but it buys that speed at the cost of stability,
and its worst case is quadratic unless mitigated (as Introsort does, by falling back to heapsort past a recursion-depth threshold —
see \S~\ref{sec:radix} below).
Timsort takes the opposite trade:
guaranteed $\textbf{O}(N \log N)$ worst case and stability,
at the cost of being merge-sort-based (extra space, and no faster than $\textbf{O}(N \log N)$ on unordered data) —
but it is adaptive,
so on data that is already close to sorted (few inversions, $I$),
its cost drops towards $\textbf{O}(N)$, which quicksort's partitioning does not exploit in the same way.
Huskysort's own strategy follows directly from this contrast:
husky-coding turns step 2's input into something quicksort (or, as described in \S~\ref{sec:radix}, radix sort)
can sort at its best case — the coded longs are effectively unordered, cheap-to-compare data —
and reserves Timsort for exactly the regime where it is strongest:
cleaning up the (hopefully few) remaining inversions in step 3, where the array is already close to sorted rather than random.
...
Huskysort is designed specifically for this use case:
unordered data consisting of objects which are relatively expensive to compare.
...
\begin{figure}
  ...
  \centering
  \includegraphics[width=0.5\linewidth]{HuskySortFlow.png}
\end{figure}
\begin{figure}
  ...
  \centering
  \includegraphics[width=0.5\linewidth]{IntroSort.png}
\end{figure}
The strategy of Huskysort (see Flow ~\ref{Fig:HS}) is as follows (assuming that the dataset is presented as an array $xs$ of type $X$):
see Algorithm ~\ref{alg:the_alg}
...
See Flow ~\ref{Fig:Introsort} \item {}Finally, when quicksort is finished and, if necessary,
...
it is not a string-sorting algorithm, but a general mechanism applicable to any \textit{Comparable} type with a 64-bit husky encoding (strings among them),
at the cost of the encoding's own fixed capture window (\S~\ref{sec:radix}
above) and a fallback cleanup pass whenever that encoding is imperfect.
We compared RadixHuskySort directly against three-way radix quicksort (JMH, English and Chinese text, natural Unicode order so both algorithms sort under the same task):
RadixHuskySort is faster at every size tested,
by 1.3--1.75x on English and 2--3.1x on Chinese, non-overlapping 99.9\% confidence intervals throughout —
...
so a fair comparison there requires a pinyin-aware MultikeyQuicksort.
We extended it with the same syllable-then-tone character key the pinyin encoding of \S~\ref{sec:radix} already uses,
restricted the comparison to the three pinyin-aware sorters (a natural-order system sort is not a real competitor for this corpus),
and re-ran it.
Across two independent runs,
confidence intervals were wide enough (up to $\pm$29\%) that the finer RadixHuskySort-vs-QuickHuskySort distinction was not resolvable —
a question the crossover analysis of \S~\ref{sec:usecase} already answers precisely on cleaner data —
but one result held directionally in both attempts:
RadixHuskySort and QuickHuskySort, both pinyin-aware, beat MultikeyQuicksort by roughly 1.6--2.7x,
consistent with the margins measured above for English and Chinese natural-order text.
...
Chinese characters using UTF8 encoding, we still find very significant gains, summarized later in Table ~\ref{tab:Improvements Summary} (\S~\ref{sec:summary}).
...
The 15/16ths comes from a final unsigned right shift by one bit, applied after packing four 16-bit characters into the 64-bit code:
without it, any string beginning with a character whose code point is 0x8000 or above (common among CJK characters) would set the sign bit and be encoded as a negative \textit{long}, corrupting simple numeric comparison.
The shift costs exactly the least-significant bit of a true fourth character;
for strings of three characters or fewer, that bit is always an unused padding zero, so nothing is actually lost — which is exactly why this encoding is only declared \textit{perfect} up to three characters, not four.
...
its in-place partitioning scans memory sequentially rather than jumping between widely-separated locations,
giving it markedly better cache behavior than, for example, heapsort\cite{LAMARCA199966}.
Cache behavior also distinguishes quicksort variants from one another, not just quicksort from other algorithm families:
dual-pivot quicksort's own performance advantage over classic single-pivot quicksort has likewise been attributed largely to cache effects\cite{Dual-pivot}.
(LaMarca and Ladner's study also found that a naive radix sort,
sorting keys directly rather than through a deferred index permutation,
has relatively poor cache behavior of its own on 1990s hardware — a finding about a different radix sort design than \S~\ref{sec:radix}'s,
on much older hardware, so it does not bear on this paper's own radix-sort results.)
So, we perform a fast quicksort on the array of longs.
The only difference between the Husky version of quicksort and standard quicksort is that the swap operation must actually do two swaps:
one of the longs and one of the object references.
...
We chose the name Husky Sort instead, after the husky, Northeastern University's mascot, where this work was developed.
...
Table ~\ref{tab:SysEnvOriginal} lists the configuration used for the JDK 1.8.0\_152-based benchmarks reported in \S~\ref{sec:analysis}
and Table ~\ref{tab:HSComp};
Table ~\ref{tab:SysEnvCurrent} lists the different configuration used for the radix-sort and JMH work of \S~\ref{sec:radix}-\S~\ref{sec:radix-results},
on different hardware and a different JVM.
A natural question is whether the qualitative finding generalizes across machines and JVM versions rather than being an artifact of one particular environment;
reporting both configurations lets a reader check directly.
The finding (Huskysort, and now radix sort,
beating System sort) holds unchanged across this substantial a difference in both machine and JVM version.

Table ~\ref{tab:SysEnvAWS} lists a third, independently run configuration used to confirm the radix-sort and parallel-scaling findings of \S~\ref{sec:radix-results} and \S~\ref{sec:parallel-radix} at larger scale: a cloud server instance which, while also ARM-based like the machine of Table ~\ref{tab:SysEnvCurrent}, uses an entirely different core design, vendor, core count, and operating system, confirmed idle before the run.
The qualitative finding holds there too, now checked across three machines that differ in vendor, architecture, and JVM build, not just two.
...
\begin{table}
  \caption{Benchmark environment used for the AWS Graviton3 confirmation run}
  \label{tab:SysEnvAWS}
  \begin{tabularx}{\linewidth}{lX}
    \toprule
...
The top-level \textit{sort} method and the default \textit{huskyEncode} method are implemented directly as shown in Algorithms ~\ref{alg:the_alg} and ~\ref{alg:the_alg2} above;
rather than repeat that logic here as Java source, we describe only the details not already covered by those two listings.

\textit{Coding} is a simple immutable holder combining the resulting \textit{long[]} array of codes with the \textit{perfect}
flag used by both algorithms (two fields, set once by its constructor).

\textit{unicodeToLong} and \textit{asciiToLong} are both thin wrappers around \textit{stringToLong}
(Algorithm ~\ref{alg:the_alg4}), differing only in which bit-width, maximum length, and mask constant they pass in;
\textit{unicodeToLong} additionally discards the lowest bit of the packed result (an unsigned right shift by one)
to guarantee a non-negative code regardless of the input string's content.
Table ~\ref{tab:EncodingConstants} lists the constants each wrapper uses.
...
\begin{figure}
  ...
  \centering
  \includegraphics[width=0.5\linewidth]{Strings.png}
\end{figure}
...
and extending it to cover radix sort (\S~\ref{sec:radix}) as an alternative to quicksort for the $T_2$ step.
...
How many comparisons and swaps does Huskysort require, on average?
It has been shown for dual-pivot quicksort [Nebel] that these values are (approximately):
\begin{itemize}
    \item Comparisons: $1.9 N ln N$;
    \item Swaps: $0.6 N ln N$.
\end{itemize}

Our experiments [refer to Quicksort Analysis table] show that each comparison requires 2 array accesses (although,
because one comparand is always a pivot value, this number is effectively closer to 1.)
...
At $N = 1{,}000{,}000$, encoding takes 52.7ms for English words (13.7\% of QuickHuskySort's 383.2ms total,
21.1\% of RadixHuskySort's 249.3ms total) and 311.6ms for Chinese names encoded via pinyin (29.1\% of QuickHuskySort's 1070.2ms total,
40.0\% of RadixHuskySort's 779.4ms total) — confirming that encoding cost, while real and non-negligible,
...
simply because radix sort's own contribution to the total is smaller —
...
Once the Huskysort is complete, we will assume that there are $p N$ remaining inversions,
...
the linear phase of Huskysort is relatively high for small $N$ but that is soon swamped by the saving of comparisons in the linearithmic phase.
...
Taking $c = 4$ (the same per-exchange cost used for Huskysort's swaps above),
...
somewhat higher than Huskysort's own break-even against merge sort ($N = 4$,
above) — because radix's per-pass cost has no logarithmic term to shrink relative to at very small $N$.
...
Table ~\ref{tab:HSComp} was generated by sorting arrays of random English words,
sampled with replacement at each size shown from the Leipzig Corpora Collection \cite{GOLDHAHN12.327},
comparing Huskysort, a re-implemented dual-pivot quicksort (see \S~\ref{sec:analysis}),
and the Java system sort (Timsort), on the environment listed in Table ~\ref{tab:SysEnvOriginal}.
...
However, as the size of the array increases beyond this value, Timsort begins to win out over insertion sort.
Please refer to table ~\ref{tab:TimvsInsertion}.
...
\begin{figure}
  ...
  \label{Fig:TvsN}
  \centering
  \includegraphics[width=0.5\linewidth]{BenchmarkTvsN.png}
\end{figure}

\item {}Comparison of Huskysort with system sort for numeric types (Integer, Double, Long, BigInteger, BigDecimal):
Table ~\ref{tab:HS_BM_N} (JMH, $N=500{,}000$, ms $\pm$ 99.9\% CI).
...
\item {}Comparison of Huskysort with system sort for Tuples:
Table ~\ref{tab:HS_BM_T} (JMH, ms $\pm$ 99.9\% CI).
...
expressed as a direct multiplier (e.g., \textbf{1.5x} means Huskysort took roughly two-thirds the time)
rather than a percentage, since a percentage figure is easy to misread:
a "42\% improvement" and a "1.7x speedup" describe the same measurement,
but only one of them is unambiguous about what it's a percentage \emph{of}.
...
Table ~\ref{tab:RadixImprovements} reports radix's advantage over QuickHuskySort as a direct multiplier —
...
This is worth confronting directly:
Table ~\ref{tab:HSComp}
shows Huskysort's own advantage over dual-pivot quicksort \emph{shrinking} as $N$ grows —
from a relative time of 0.60 at $N=1{,}000$ to 0.97 at $N=4{,}000{,}000$,
i.e., the improvement fades from roughly 40\% to roughly 3\%, which would undermine confidence that the advantage is durable at larger scale.
Radix sort's advantage over QuickHuskySort does not shrink with $N$ in the same way:
at every size and data type in Table ~\ref{tab:RadixImprovements}, the margin is 1.3x or greater,
...
because both QuickHuskySort and RadixHuskySort must pay the same collator-based cleanup pass cost for that coding;
Dates show the largest margin (4.5x) because that coding is provably perfect and never needs a cleanup pass at all,
letting radix's linear-time advantage show through with nothing else in the way.

An independent confirmation run on entirely different hardware (Table ~\ref{tab:SysEnvAWS}) reinforces this finding:
at $N=1{,}000{,}000$, RadixHuskySort/16 beat QuickHuskySort by 2.51x on English text and 2.85x on Chinese text,
with confidence intervals under 2\% of the mean even at that size —
a margin two orders of magnitude larger than the measurement noise, on hardware entirely different from either machine of \S~\ref{sec:analysis} above.
...
gives a real, statistically resolved speedup at both sizes:
1.41x at $N=2{,}000{,}000$ and 1.61x at $N=10{,}000{,}000$.
These are non-overlapping confidence intervals in both cases.
Compared with the serial implementation, the improvement is resolved at $N=10{,}000{,}000$ (574.3ms to 426.4ms, 1.35x)
and is directionally consistent but not fully resolved at $N=2{,}000{,}000$ (1.32x, overlapping intervals).
Four threads is the practical sweet spot on the machine used for this measurement (four performance and four efficiency cores):
eight threads performs statistically indistinguishably from four at both sizes,
consistent with only four of the eight cores being full-performance ones.

A follow-up confirmation run rules out that explanation, though:
on a cloud server instance with 16 uniform cores and no performance/efficiency split (Table ~\ref{tab:SysEnvAWS}),
scaling from $p=1$ to $p=8$ threads at $N=10{,}000{,}000$ was 980.9ms to 672.5ms —
only 1.46x for an 8x increase in thread count, essentially the same flattening pattern seen above despite there being no core asymmetry to blame it on.
The parallel variant still beat QuickHuskySort by roughly 6-9x on that run even at this weak scaling,
since most of that margin comes from radix sort's own serial advantage rather than parallelism's incremental contribution.
This points instead to a property of the design itself:
each pass's sequential histogram-combine step runs once per synchronization point regardless of thread count,
so it becomes a proportionally larger part of the total as thread count grows — an Amdahl's-law-style bottleneck rather than an artifact of any one machine's core layout.
...
which sorter to reach for still depends on $N$.

For String keys we measured every crossover directly (JMH, English corpus, $N=4$ through $10{,}000$).
System sort is the fastest of the group at $N=20$ and $N=50$ (non-overlapping 99.9\% confidence intervals against QuickHuskySort),
essentially tied with QuickHuskySort by $N=100$--$200$, and clearly overtaken by QuickHuskySort from $N=500$ on.
A plain, binary-search-based insertion sort is statistically indistinguishable from System sort at $N \le 20$,
but wins outright over both System sort and QuickHuskySort from roughly $N=100$ through $N=200$
(non-overlapping confidence intervals both ways) — the one regime in this paper where plain insertion sort,
not either Huskysort variant, is the best available choice.
QuickHuskySort pulls back ahead of insertion sort by $N=500$, and remains the best choice up to roughly $N=2{,}000$,
since RadixHuskySort's own fixed digit-pass setup cost is not yet amortized below that point.
Only somewhere between $N=2{,}000$ and $N=5{,}000$ does RadixHuskySort take over;
...
where a parallel radix sort's own thread/barrier setup only pays off once there is enough work to divide across threads.
...
so other key types plausibly show similarly-shaped curves at similar sizes, but this has not been separately confirmed.
...
\begin{figure*}
  ...
  \caption{Which sort to reach for, by array size (String keys).
Band edges fade across the
    measured crossover zones rather than showing a hard cutoff, reflecting the genuine
    uncertainty in exactly where each crossover falls.
Not shown:
if the source data defeats the
...
(Tables ~\ref{tab:SysEnvOriginal}, ~\ref{tab:SysEnvCurrent}, and ~\ref{tab:SysEnvAWS}),
...
HuskySort's advantage is specific to a particular kind of workload, not universal:
a large array — at least a few hundred elements for QuickHuskySort,
growing to a few thousand or more before RadixHuskySort's own fixed per-pass setup cost is worth paying (\S~\ref{sec:usecase}) —
of objects with no expectation of pre-existing order
(a partially-sorted array favors Timsort's adaptivity instead, \S~\ref{sec:summary}),
whose comparison or equality test costs more than a single machine-word check —
Strings, arbitrary-precision numbers, and multi-field records,
as opposed to \textit{int}/\textit{long}/\textit{double}, where dual-pivot quicksort already wins outright.
...
and compared against a re-implementation from scratch of dual-pivot quicksort (the same baseline used for the numeric comparisons above,
without any three-way/equal-key handling).
Table ~\ref{tab:AdversarialBits} reports timings (milliseconds) at $N = 1{,}000{,}000$.
...
The from-scratch quicksort baseline degrades by more than an order of magnitude and then fails outright with a stack overflow —
a concrete instance of exactly the kind of adversarial input this concern anticipates,
though notably it is the \emph{baseline} implementation that fails, not QuickHuskySort itself.
This is a well-known failure mode for a quicksort implementation that partitions naïvely around a heavily-duplicated pivot value:
...
neither crashes nor slows down anywhere in this sweep — if anything it gets faster as duplication increases,
...
Radix sort is more robust still, because its immunity to this pattern is structural rather than incidental to a fallback mechanism.
...
radix included, ends up slower than plain System sort, not merely no-longer-faster.
...
radix sort is effectively immune, and a naively-implemented quicksort baseline is not —
it can degrade by orders of magnitude or fail outright.
...
RadixHuskySort was never at risk of this particular failure mode in the first place,
...
the merge-sort variation improves on system sort by approximately 20 percent whereas the normal intro-sort-based Huskysort improves on the system sort by more like 40 Percent.

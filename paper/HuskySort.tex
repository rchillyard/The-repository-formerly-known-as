...
We chose the name Husky Sort instead, after the husky, which is the mascot of the university where this work was developed.
...
\S~\ref{sec:permits} gives the case study.
...
because the encoding for them is exact and can be declared so ---
which is what allows the pass to be switched on and off over identical codes.

\subsection{Composite keys, and what a library would have to provide}
\label{sec:composite-future}

The permit coder above was written by hand, as was every composite encoding in this paper,
and one practical obstacle stands between that and a library implementation of this mechanism.

...
Its second phase was a comparison sort, so the $\textbf{O}(N \log N)$ term survived the encoding intact;
and the cost of an inexact encoding, which every string corpus incurs, was argued rather than measured.
...
The code need not be exact, only good: whatever inversions it leaves are the cleanup pass's problem, and
\S~\ref{sec:pcrit} measures what they cost.

Exactness has a precise limit for strings.
...
Whether a coding was in fact exact is known before the cleanup pass runs: a constant-time lookup for
non-sequence types, and for strings a scan that stops at the first string too long to encode exactly.
Where it was exact, no inversions remain and the pass is skipped altogether.

...
rather than on radix sort, so they are in \S~\ref{sec:comparison-baselines}.
...
\begin{table*}
  ...
  \begin{tabular}{llllll}
    \toprule
    Data type & N & System sort & QuickHuskySort & Radix/8 & Radix/16 \\
    \midrule
...
These figures are from machine A of Table ~\ref{tab:SysEnv} and are the paper's only measurements not
...
quicksort adapted to objects, and a from-scratch quicksort with no equal-key handling.
Both bear on QuickHuskySort rather than on radix sort, which is why they are here rather than in the
body.
...
\begin{table*}
  \caption{Comparison-sort baselines (JMH, ms, mean $\pm$ 99.9\% CI). Numeric types at
    $N=500{,}000$. A from-scratch quicksort was measured for the numeric types only.}

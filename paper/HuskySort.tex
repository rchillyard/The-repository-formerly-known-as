...
\begin{table*}
  ...
  \begin{tabular}{p{0.42\textwidth}p{0.24\textwidth}p{0.26\textwidth}}
    ...
    The ordering is already cheap to evaluate --- primitives, or a single numeric field
      & dual-pivot quicksort & the encoding cannot repay itself (\S~\ref{sec:summary}) \\
    The ordering is composite or expensive, \emph{and} the key packs exactly into 64 bits
      & RadixHuskySort; no cleanup pass runs at all
    ...
    The ordering is composite or expensive, but the encoding is imperfect
      & RadixHuskySort still, allowing for the cleanup pass
      & that pass costs 10--25\% even when it has nothing to correct (\S~\ref{sec:radix-results}) \\
    A sort specialised to your type already exists
      & use it & on English words a tuned MSD radix sort beats us (\S~\ref{sec:radix}) \\
    ...
    Fewer than a few thousand elements
      & see Figure ~\ref{fig:usecase} & fixed setup costs dominate below that \\
    \bottomrule
  \end{tabular}
\end{table*}
...
\begin{figure*}
  ...
  \caption{Where the fixed setup costs put the early crossovers, for String keys.
This answers the last row of Table ~\ref{tab:Guidance} only:
it compares the system sort with the husky family, and does not include a sort specialised to strings.
At $N \ge 200{,}000$ on English text a tuned MSD radix sort outperforms every algorithm shown
(\S~\ref{sec:radix}), which is the fourth row of that table rather than this one.

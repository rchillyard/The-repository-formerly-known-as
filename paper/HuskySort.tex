...
\begin{table*}
  ...
  \begin{tabular}{p{0.42\textwidth}p{0.24\textwidth}p{0.26\textwidth}}
    ...
    A sort specialised to your type already exists
      ...
      & on English words a tuned MSD radix sort beats us, though ours indexes extended ASCII only
        and cannot sort either Chinese corpus (\S~\ref{sec:radix}) \\
    ...
    None of the above settles it, or the keys are strings
      & see Figure ~\ref{fig:usecase} & where each fixed setup cost is repaid, for English text \\
    \bottomrule
  \end{tabular}
\end{table*}
...
Band edges fade across the interval each crossover is bracketed within.
...
If instead the data defeats the encoding's capture window,

...
\begin{abstract}
    ...
    it nonetheless accounts for as much as a fifth of total running time.
...
This paper builds on the work of Hillyard et al.\cite{huskysort2020},
which introduced the husky code and the QuickHuskySort algorithm that sorts by it.
The contributions here are RadixHuskySort (\S~\ref{sec:radix}),
...
The encoding for these records is exact, so the pass that runs has provably nothing to correct.
...
That comparison costs 12.7\,ns at $N = 32{,}000$, 68.2\,ns at $N = 100{,}000$ and 85.1\,ns at
$N = 198{,}900$ --- 5.8\%, 18.6\% and 19.5\% of the running time of the sort that contains it ---
with non-overlapping 99.9\% confidence intervals throughout.
Two further runs, one on different hardware, agree on the shape at different levels
(20.0/74.1/68.7 and 8.0/22.1/37.8\,ns).
The percentages are the less stable figures, being ratios of two quantities that both move.
The shape is a threshold rather than a trend --- a rise by a factor of roughly three to five between the first two sizes,
...
the system sort compares these same records some 3.5 million times in 162\,ms, or 46\,ns each,
so the cleanup pass at 85\,ns costs nearly twice as much per comparison using the same comparator.
Step 2 permutes references without moving the records,
so a scan sequential in the array is close to random in the heap.

One objection to these figures should be met head on:
the pass is linear where the sort containing it is linearithmic,
so its share of the total is bound to fall as $1/\log N$, and the shares above instead rise.
Both are true, because $k_3$ is not settled over this range.
It grows almost sevenfold between the smallest and largest sizes here while $\ln N$ grows by a fifth,
so the cache effect swamps the logarithm completely.
Holding $k_3$ at the value it reaches once the records no longer fit in cache,
and growing the rest as $\ln N$, puts the share at some 18\% at $N = 10^{6}$ and 15\% at $N = 10^{7}$:
...
First, $k_3$ is not a constant: it varies almost sevenfold across three sizes on one machine,
so no single $p_{crit}$ can be quoted even for one type on one machine.
Second, an encoding which can be declared perfect avoids a cost of up to a fifth of total running
...
Your situation & Reach for & Evidence \\
    ...
    The ordering is composite or expensive, but the encoding is imperfect
      ...
      & that pass costs a sixth to a fifth past $N=100{,}000$, even with nothing to correct
        (\S~\ref{sec:pcrit}) \\

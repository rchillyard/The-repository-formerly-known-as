...
\begin{abstract}
    ...
    it nonetheless accounts for as much as a quarter of total running time.
...
%% The draft revision stamp declared in the preamble, added to that bare page number.
%% NOTE the whole page style has to be redefined, not just the footer: both pagestyle and
%% thispagestyle re-execute \ps@plain when the page is shipped out, which would overwrite a
%% bare \@oddfoot patch made here. Hence the redefinition, then re-applying it.
%% The phantom on the right keeps the page number centred rather than nudged off-centre.
\ifdraftstamp
  \makeatletter
  \def\ps@plain{%
    ...
    \let\@oddhead\@empty
    \let\@evenhead\@empty
    %% The page number keeps the whole width to centre itself in; the stamp is then lapped
    %% leftward from the right margin, so it cannot shift the number off-centre.
    \def\@oddfoot{\reset@font\hfil\thepage\hfil
      \llap{\footnotesize\itshape draft \revisionstamp}}%
...
They span from 3.3x down to 1.5x, nearly the whole of its range below dates,
...
Chinese personal names are unfavourable on the second: the native comparison is more expensive still,
...
consumes what the encoding saves, and this is the weakest result we report.
...
HuskySort sorts in 732 ms against the system sort's 1,194 ms, a factor of 1.63
(for the full data, please refer to table ~\ref{tab:HS_BM_S}).
...
Chinese characters using UTF8 encoding, we still find very significant gains, reported later in Table ~\ref{tab:HS_BM_S}.

Of the three terms, $T_3$ is the one we can measure in isolation,
and doing so puts a floor under what an imperfect encoding costs.
Two benchmarks over the same two hundred thousand municipal records (\S~\ref{sec:radix-results})
differ in one respect only: which coder they are given.
Both obtain their codes from the same method, so the codes are identical;
one coder declares itself perfect and the sort therefore skips step 3,
while the other leaves that declaration at its default of false and the pass runs.
Since the encoding for this data is exact,
the pass that does run provably has nothing to correct,
and the difference between the two benchmarks is $T_3$ evaluated at $p = 0$ --- overhead and nothing else.
It is not small.
On the machine of record it accounts for 9.8\% of total running time at $N = 32{,}000$,
25.3\% at $N = 100{,}000$ and 19.0\% at $N = 198{,}900$,
with non-overlapping 99.9\% confidence intervals at every size.
A second, independent run on the same machine gave 6.2\%, 22.8\% and 24.2\%.

We therefore draw only the conclusion both runs support:
that this pass costs under a tenth of total running time at the smallest size measured
and between a fifth and a quarter at the two larger ones,
while doing no useful work at all.
We specifically do not claim a trend in $N$.
The two runs disagree about the direction beyond $N = 100{,}000$ --- one rises, the other falls ---
and a third run on different hardware produced a third shape again,
so the magnitude of this cost replicates and its shape does not.
This is also, to our knowledge, the only direct measurement of the quantity,
every other benchmark in this paper varying the encoding and the sort together.

It is what makes $p_{crit}$ worth establishing rather than merely bounding.
An encoding that can be declared perfect avoids this cost outright;
one that cannot must first repay it, before a single genuine inversion has been corrected.
...
Table ~\ref{tab:TimvsInsertion} among them;
...
One is not.
The building-permit records of \S~\ref{sec:radix-results} are 198,900 rows of a public municipal dataset,
each ordered by a composite key of three fields --- assessor's block, assessor's lot, and filing date ---
and they are included precisely because they are real,
with whatever distribution of block sizes, lot numbering and filing dates the city happens to have.
...
Please refer to tables ~\ref{tab:HS_BM_N} and ~\ref{tab:HS_BM_T}, which carry a dual-pivot column.
...
Typical improvements over Timsort can be read directly off Tables ~\ref{tab:HS_BM_N}, ~\ref{tab:HS_BM_S}
and ~\ref{tab:HS_BM_T} above, which give absolute times with 99.9\% confidence intervals rather than
summary ranges:
1.4--1.8x across the five numeric types at $N=500{,}000$,
1.3--1.6x on English words,
2.0--2.3x on Chinese words,
and 1.3--1.4x on tuples.
Note that the English margin \emph{widens} with array size rather than decaying,
from 1.33x at $N=32{,}000$ to 1.63x at $N=1{,}000{,}000$.
...
\begin{table*}
  ...
  \begin{tabular}{cll}
    ...
    \texttt{Integer} & 500,000 & 3.4x \\
    \texttt{Strings of Chinese words (natural order)} & 1,000,000 & 3.3x \\
    \texttt{Double} & 500,000 & 3.0x \\
    \texttt{BigDecimal} & 500,000 & 2.9x \\
    \texttt{BigInteger} & 500,000 & 2.8x \\
    \texttt{Strings of English words} & 1,000,000 & 2.7x \\
    \texttt{Tuples} & 500,000 & 2.5x \\
    \texttt{Building permits (composite key)} & 198,900 & 2.1x \\
    \texttt{Strings of Chinese names (pinyin order)} & 1,000,000 & 1.5x \\
    \bottomrule
  \end{tabular}
\end{table*}
...
from 1.33x at $N=32{,}000$ to 1.53x and then 1.63x (Table ~\ref{tab:HS_BM_S}),
and holds roughly constant on Chinese.
...
Chinese names sorted by pinyin order are the smallest margin measured (1.5x),
...
The two lie at opposite ends of the same axis: what separates them is not the data type but the exactness of the encoding.

One row of that table deserves separating from the rest, because it is the only one not measured on data we generated.
The building permits are 198,900 real municipal records ordered by a composite key of three fields
(\S~\ref{sec:data-source}), and they are the case this mechanism is designed for:
the ordering is expensive, since a comparison may have to examine all three fields in turn,
and the key nevertheless packs exactly into 60 bits, so no cleanup pass is required at all.
Against the system sort, RadixHuskySort is faster by 5.25x, 4.53x and 4.77x
at $N = 32{,}000$, $100{,}000$ and $198{,}900$;
against QuickHuskySort, which Table ~\ref{tab:RadixImprovements} reports, by 2.56x, 2.17x and 2.06x.
At 2.1x the permits row is tenth of eleven, and that is the point worth making about it.
It is not our largest margin --- a composite key of three fields is more expensive to compare
than a single \textit{Long} but a good deal cheaper than a pinyin lookup per character ---
but it is our largest margin on data whose distribution we did not choose,
and it falls squarely inside the range predicted for it by the three factors.
A mechanism that only worked on synthetic inputs would not be worth reporting.
...
serial vs. 1, 2, 4, and 8 worker threads}
  ...
  \begin{tabular}{cccccc}
    ...
    \midrule
    2,000,000 & 174.0$\pm$6.0 & 170.2$\pm$2.2 & 139.3$\pm$1.4 & 121.7$\pm$1.8 & 110.5$\pm$2.6 \\
    10,000,000 & 943.4$\pm$31.4 & 887.6$\pm$25.7 & 745.3$\pm$21.6 & 685.4$\pm$22.4 & 634.1$\pm$23.2 \\
    \bottomrule
  \end{tabular}
\end{table*}
...
gives a statistically resolved speedup at both sizes: 1.40x at $N=2{,}000{,}000$ and 1.29x at $N=10{,}000{,}000$,
...
174.0ms to 121.7ms at $N=2{,}000{,}000$ and 943.4ms to 685.4ms at $N=10{,}000{,}000$.
...
reaching 1.54x and 1.40x over a single thread.
...
The explanation we favour is bandwidth rather than scheduling:
each digit pass scatters across the whole array, so the algorithm is limited by memory throughput,
which extra cores do not supply.
...
\begin{table*}
  \caption{Which sort to reach for. The questions are ordered by how much they decide:
    the cost of the comparison, the exactness of the encoding, and the existence of a sort
    specialised to the type. Array size settles nothing until all three are answered.}
  \label{tab:Guidance}
...
Your situation & Reach for & Evidence \\
    ...
    The ordering is composite or expensive, but the encoding is imperfect
      ...
      & that pass costs a fifth to a quarter past $N=100{,}000$, even with nothing to correct
        (\S~\ref{sec:pcrit}) \\
...
\begin{table*}
  ...
  \begin{tabular}{ccccc}
    ...
    60 & 115.3 & 67.7  & \textit{crashed} & 44.4 \\
    63 & 43.3  & 15.5  & \textit{crashed} & 35.0 \\
    \bottomrule
  \end{tabular}
\end{table*}
...
Indeed both overtake radix sort at the extreme of the sweep,
...
Radix sort's robustness is of a different kind, and the more useful one:
its cost is flat because it does not depend on the distribution at all,
so it is the only algorithm here whose worst case can be read off its best.

...
Both consistently outperform Java's own system sort for objects —
Timsort — where the objects are expensive to compare, such as Strings:
...
Consequently, there is an abundance of sorting techniques available for a myriad of different use cases.
...
the AWS run of \S~\ref{sec:radix-results}--\S~\ref{sec:parallel-radix} only tested $N \ge 32{,}000$,
...
All three of the comparison baselines discussed here therefore carry an unbounded-recursion failure mode:
we repaired two of them, and the third is reported above as a result in its own right.

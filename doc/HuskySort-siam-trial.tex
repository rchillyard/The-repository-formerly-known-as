...
%% SHIM: siamproceedings.sty (Jan 2025) calls \AddToHook, which arrived in the LaTeX kernel of
%% 2020-10-01. This machine has TeX Live 2020, kernel 2020-02-02, so it is undefined here. All ten
%% uses in that file are \crefalias declarations for theorem-like environments (lemma, corollary,
%% proposition, definition, claim, fact) which this paper does not use, so discarding them costs
%% nothing. Remove this shim when building on a current TeX distribution.
\makeatletter
...
%------------------------------------------------------------------------------
%% ORCID icon machinery and the three author ORCIDs removed: anonymous submission (0k).
%% NOTE they were already inside a `comment` environment in the acmart source, so inert.
%%

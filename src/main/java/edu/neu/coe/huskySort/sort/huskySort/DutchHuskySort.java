...
/**
 * Class to define HuskySort which uses plain quicksort with three-way (Dutch national flag)
 * partitioning, and no depth-limit or heapsort fallback -- unlike {@link QuickHuskySort}, which
 * adds that fallback (Introsort, per Musser 1997) to guard against the quadratic worst case.
 *
 * @param <X> the underlying type of element to be sorted.
 */
public final class DutchHuskySort<X extends Comparable<X>> extends AbstractHuskySort<X> {
     ...
     */
    public DutchHuskySort(final String name, final int n, final HuskyCoder<X> huskyCoder, final Consumer<X[]> postSorter, final Config config) {
     ...
     * @param config     the configuration.
     */
    public DutchHuskySort(final String name, final HuskyCoder<X> huskyCoder, final Consumer<X[]> postSorter, final Config config) {
        this(name, 0, huskyCoder, postSorter, config);
    }

    /**
     * Secondary constructor to create an implementation of HuskySort which primarily uses Quicksort.
     * The name will be DutchHuskySort/System.
     * The post-sorter will be the System sort.
     *
     * @param huskyCoder the Husky coder.
     * @param config     the configuration.
     */
    public DutchHuskySort(final HuskyCoder<X> huskyCoder, final Config config) {
        this("DutchHuskySort/System", huskyCoder, Arrays::sort, config);
    }
